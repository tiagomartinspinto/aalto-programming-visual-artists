...
function loadStudentStudies(filePath) {
  try {
    const source = readFileSync(filePath, "utf8");
    const context = { window: {} };
    vm.createContext(context);
...
function checkCounts() {
  ...
  for (const year of countDirectories(path.join(root, "years")) ? readdirSync(path.join(root, "years")) : []) {
    ...
    const studentDataPath = path.join(yearPath, "case-studies", "studies.js");
    const studentStudies = existsSync(studentDataPath) ? loadStudentStudies(studentDataPath) : [];
    const studies = studentStudies.length || countDirectories(path.join(yearPath, "case-studies"), (name) => /^(study|assignment)-/i.test(name));
    ...
    const caseStudyIndex = path.join(yearPath, "case-studies", "index.html");
    if (existsSync(caseStudyIndex)) {
      const caseHtml = readFileSync(caseStudyIndex, "utf8");
      if (!cspPattern.test(caseHtml)) errors.push(`years/${year}/case-studies/index.html is missing a Content-Security-Policy meta tag`);
      if (studies > 0 && !caseHtml.includes(`${studies} course studies`)) {
        errors.push(`years/${year}/case-studies/index.html has a stale student-study count; expected "${studies} course studies"`);
      }
    }
...
for (const filePath of walk(root)) {
  ...
  if (/\.(?:html|js|css|md|txt|pde|rb|json|yml|yaml)$/i.test(filePath)) {
    ...
    if (rel.includes("/case-studies/") && !rel.includes("/vendor/") && (content.includes("mailto:") || emailPattern.test(content))) {
      errors.push(`${rel} contains an email address inside student study material`);
    }
  }
